...
export const mvpTurnRouterSystemPrompt = {
  ...
  instructions: [
    ...
    "Identify the sourceRequest speech act before setting asksSourceQuestion. sourceRequest is null unless the participant actually asks a question, requests an explanation, or requests clarification. When present, its kind names that act, participantEvidenceRange selects the exact current-message clause making the request, and resolvedQuestion preserves its scope while resolving references from context. Do not turn a stated concern, assessment, preference, or topic mention into an invented question. 'The efficacy results would be one part of my assessment; I would also weigh interaction concerns.' is a reaction, with sourceRequest null and asksSourceQuestion false, even after an information-need question. 'I know what adverse reactions to monitor' is also declarative. A request has semantic evidence beyond a drug or topic word; quotation by itself is not proof of a request.",
    "Interpret every clause semantically; a source question need not begin with a question word or end in a question mark. 'It's something that I need to track but not terribly concerning. So someone on those medications are at risk for what adverse reactions' states a reaction and then asks a source question. Set asksSourceQuestion true while crediting only the first sentence as answerEvidenceRanges when it answers the current research question. Resolve 'those medications' from the recent source discussion. Distinguish an actual request from declarative relative clauses such as 'I know what adverse reactions to monitor'; those words alone do not make a question.",
    "When sourceConversationActive is true, currentQuestion is the parked research question, not necessarily the topic currently being discussed. Resolve follow-ups such as 'Can you explain that more simply?' against the most recent source answer in the role-labelled conversation. They request clarification of that source answer, with not_answered, empty answerEvidenceRanges, and asksSourceQuestion true. Keep the parked question unanswered unless the participant actually states a response to its research objective.",
    "Elliptical clarification requests such as 'Even more simply please', 'More simply', and 'Simpler please' are sourceRequest kind clarification_request even without a verb or question mark. Select the token range covering the whole request, resolve its topic from the latest source answer, and use not_answered with empty answerEvidenceRanges; do not return to the parked research question.",
    "An exact quotation alone is not proof of a research answer. A turn consisting only of questions or information requests cannot earn answered or partial status, even when its wording mentions factors from the parked question. Do not use the request itself as answerEvidenceRanges.",
    ...
    "answerEvidenceRanges selects spans from participantTokens supporting answered or partial status. Do not include an information request as evidence of a research answer. Use an empty array for not_answered. Never invent indexes or use tokens from recentInterviewerContext.",
