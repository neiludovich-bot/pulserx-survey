...
import { sourceRequestSchema, mvpTurnRouteAnalysisModelResultSchema, mvpTurnRouteAnalysisResultSchema, type MvpTurnRouteAnalysisInput } from "../../schemas/src/index";
...
describe("typed participant source requests", () => {
  it("requires a strict nullable speech-act record in new model output while preserving older application records", () => {
    const format = zodTextFormat(mvpTurnRouteAnalysisModelResultSchema, "route");
    expect(format.schema.required).toContain("sourceRequest");
    expect(mvpTurnRouteAnalysisModelResultSchema.safeParse({ ...result, sourceRequest: undefined }).success).toBe(false);
  ...
  it("retains a declarative reaction and rejects a positive source flag without an actual request record", async () => {
    expect((await gateway(result).analyzeMvpTurnRoute(input)).result).toMatchObject({ asksSourceQuestion: false, sourceRequest: null, answerEvidence: [reaction] });
    await expect(gateway({ ...result, asksSourceQuestion: true, needsSource: true }).analyzeMvpTurnRoute(input)).rejects.toThrow("source request");
  });
  ...
  it("retains exact mixed-turn reaction and embedded question without requiring punctuation", async () => {
    ...
    const routed = await gateway({ ...result, asksSourceQuestion: true, needsSource: true, kind: "source_question", sourceRequest, sourceDirective: "Answer the participant's follow-up." }).analyzeMvpTurnRoute({ ...input, participantMessage: `${reaction} ${question}` });
  ...
  it("rejects an invented source question excerpt instead of fabricating a request from a concern", async () => {
    await expect(gateway({ ...result, asksSourceQuestion: true, needsSource: true, sourceRequest: { kind: "question", participantEvidence: "What are the interactions?", resolvedQuestion: "What are the interactions?" } }).analyzeMvpTurnRoute(input)).rejects.toThrow("exact current participant excerpts");
  });
});
