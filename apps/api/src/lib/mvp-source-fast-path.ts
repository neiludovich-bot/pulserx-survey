...
/** Whole-message navigation also advances an already presented priority. */
export function sourceDiscussionFastPath(state: ModeratorState, message: string, isResumeCue: boolean) {
  ...
  if (!discussion && !(isResumeCue && state.priorities.some(p => ["pending", "presented"].includes(p.status)))) return null;
  ...
  const plan = moderatorPlanResultSchema.parse({ newPriorities: [], reactionStatus: "not_answered", reactionEvidence: [], sourceRequest,
    action: kind !== "resume" ? "answer_source" : !discussion && pending ? "present_priority" : "resume_guide",
    selectedPriorityId: kind !== "resume" ? active?.id ?? null : !discussion ? pending?.id ?? null : null, rationale });
