...
import { sanitizeMvpRouteFailure } from "./mvp-openai-turn-router";
...
export function logSyntheticModeratorDecision(input: Input): void {
  ...
  try {
    ...
    console.warn(JSON.stringify({
      ...
      router: { provider: ["openai_hybrid", "deterministic"].includes(input.route.provider) ? input.route.provider : "unknown", answerStatus, asksSourceQuestion: Boolean(input.route.asksSourceQuestion), sourceRequestProvided: input.route.sourceRequest !== undefined, sourceRequest: request(input.route.sourceRequest), answerSpans: spans(input.route.answerEvidence), modelAttempts: typeof input.route.modelAttempts === "number" ? Math.max(0, Math.min(2, input.route.modelAttempts)) : null, modelFailures: Array.isArray(input.route.modelFailures) ? input.route.modelFailures.slice(0, 2).map(sanitizeMvpRouteFailure) : [], failureDiagnosis: input.route.failureDiagnosis ? sanitizeMvpRouteFailure(input.route.failureDiagnosis) : null, skipReason: input.route.skipReason && ["provider_disabled", "no_candidates", "out_of_scope", "consent", "empty_message", "gateway_unavailable"].includes(input.route.skipReason) ? input.route.skipReason : null },
