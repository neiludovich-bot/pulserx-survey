...
export function websitePageContext(content: string, matchedText: string, broad: boolean) {
  ...
  const anchor = broad ? 0 : anchors.map(text => content.indexOf(text)).find(index => index >= 0) ?? 0;
