import { downloadPublicWebsite } from "./website-download";
import { load } from "cheerio";
import { createHash } from "node:crypto";
import { websiteIndexSnapshotSchema, WEBSITE_PROFILES, allowedWebsiteIndexUrl, type WebsiteIndexSnapshot, type WebsiteProfile } from "@interview/schemas";
...
export async function download(slug: WebsiteIndexSnapshot["surveySlug"], value: string, profile?: WebsiteProfile) {
  let url = value;
  for (let redirects = 0; redirects < 6; redirects++) {
    if (!allowedWebsiteIndexUrl(slug, url, true, profile)) throw new Error("Redirect or URL outside approved website/document hosts");
    const response = await downloadPublicWebsite(url);
    if (response.status >= 300 && response.status < 400 && response.location) { url = canonicalUrl(response.location, url)!; continue; }
    if (response.status < 200 || response.status >= 300) throw new Error(`HTTP ${response.status}`);
    return { url, bytes: response.bytes, type: response.type };
  }
...
export async function indexMedicalWebsite(slug: WebsiteIndexSnapshot["surveySlug"], configuredProfile?: WebsiteProfile) {
  ...
  const enqueue = (url: string, from: string) => {
    if (!allowedWebsiteIndexUrl(slug, url, true, profile) || /\.(?:svg|png|jpg|jpeg|gif|webp|zip|mp4|css|js|xml)$/i.test(new URL(url).pathname)) return;
  ...
  for (const source of CONTROLLED_RAG_CHUNKS.filter(s => s.surveySlug === slug)) {
    ...
    if (url && allowedWebsiteIndexUrl(slug, url, false, profile)) enqueue(url, profile.rootUrl);
  }
  ...
  while (sitemapQueue.length && sitemapSeen.size < 12) {
    ...
    if (sitemapSeen.has(url) || !allowedWebsiteIndexUrl(slug, url, false, profile)) continue;
    sitemapSeen.add(url);
    try {
      const map = await download(slug, url, profile); const xml = load(map.bytes.toString("utf8"), { xmlMode: true });
  ...
  while (queue.length && processed < 180) {
    ...
    try {
      const resource = await download(slug, item.url, profile); seen.add(resource.url);
